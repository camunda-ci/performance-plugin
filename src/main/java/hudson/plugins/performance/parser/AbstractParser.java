...
import static hudson.plugins.performance.util.AssertHelper.checkNotNull;
...
{  
  ...
  public Collection<PerformanceReport> parse(AbstractBuild<?, ?> build, Collection<File> reports, TaskListener listener) throws IOException {
    ...
    for (File reportFile : reports) {
      ...
      // When serialized data cannot be used, the original files are to be processed.
      try {
        listener.getLogger().println("Performance: Parsing " + getReportName() + " report file '" + reportFile + "'.");
        final PerformanceReport report = parse(reportFile);
        result.add(report);
        saveSerializedReport(reportFile, report);
      } catch (Throwable e) {
        listener.getLogger().println("Performance: Failed to parse " + getReportName() + " file '" + reportFile + "': " + e.getMessage());
   ...
   *           On any exception.
   */
  public abstract PerformanceReport parse(File reportFile) throws Exception;
  ...
  protected static PerformanceReport loadSerializedReport(File reportFile) {
    checkNotNull("Argument 'reportFile' cannot be null.", reportFile);
  ...
  protected static void saveSerializedReport(File reportFile, PerformanceReport report) {
    checkNotNull("Argument 'reportFile' cannot be null.", reportFile);
    checkNotNull("Argument 'report' cannot be null.", report);
    final String serialized = reportFile.getPath() + SERIALIZED_DATA_FILE_SUFFIX;
