...
public class PerformanceReportTest {
  ...
  public void testAddSample() throws Exception {
    ...
    EasyMock.expect(
        performanceReport.getBuildAction().getConsoleWriter())
    ...
    HttpPerformanceReportSample sample1 = new HttpPerformanceReportSample();
  ...
  */
  ...
  private PerformanceReport parseOneJMeter(File f) throws IOException {
    return new JMeterParser("").parse(null, Collections.singleton(f), StreamTaskListener.fromStdout()).iterator().next();
  }

  private PerformanceReport parseOneJUnit(File f) throws IOException {
    return new JUnitParser("").parse(null, Collections.singleton(f), StreamTaskListener.fromStdout()).iterator().next();
  }
  ...
  public void testIssue5571() throws IOException, SAXException {
    PerformanceReport performanceReport = parseOneJUnit(new File("src/test/resources/jUnitIssue5571.xml"));
    Map<String, UriReport> uriReportMap = performanceReport.getUriReportMap();
    assertEquals(1, uriReportMap.size());
  ...
  public void testPerformanceReportMultiLevel() throws IOException, SAXException {
    PerformanceReport performanceReport = parseOneJMeter(new File("src/test/resources/JMeterResultsMultiLevel.jtl"));
    Map<String, UriReport> uriReportMap = performanceReport.getUriReportMap();
    assertEquals(2, uriReportMap.size());
